...
def setStartFrequency(startFrequency):
    ...
    fullCommand = (
        "\
    delta=1;\
    frequency=`grep FINALFREQ "
        + LFmapPath
        + "LFmap.config | awk '{print $2}'`;\
    echo $frequency;\
    new_frequency="
        + startFrequency
        + ';\
    echo $new_frequency;\
    sed -i "s|$frequency|$new_frequency|" '
        + LFmapPath
        + "/LFmap.config;\
    "
    )
...
def setOUTFORM():
    fullCommand = (
        '\
    outformVar=`grep "OUTFORM" '
        + LFmapPath
        + "/LFmap.config |awk '{print$2}'`;\
    echo Chaning OUTFORM from $outformVar to 4;\
    sed -i \"s|OUTFORM $outformVar|OUTFORM 4|\" "
        + LFmapPath
        + "/LFmap.config;\
    "
    )
...
def main(startFrequency="30.0", endFrequency="80.0", fdelta=0.1, path="pylfmap/LFmap/"):
    ...
    while currentFrequency <= float(endFrequency):
        p = subprocess.run(
            cd_to_lfmap_folder + " ./LFmap", shell=True, capture_output=True
        )
        print(p.stdout.decode())
        print(p.stderr.decode())
        currentFrequency = (
            float(p.stdout.decode().splitlines()[1].split(" ")[-1]) + fdelta
        )
    ...
    res = subprocess.run(
        "mkdir " + LFmapPath + "/healpyFits", shell=True, capture_output=True
    )
    print(res.stdout.decode())
    print(res.stderr.decode())
    res = subprocess.run(
        "mkdir " + LFmapPath + "/fits", shell=True, capture_output=True
    )
    print(res.stdout.decode())
    print(res.stderr.decode())
    print("Moving generated *_healpy.fits files to " + LFmapPath + "/healpyFits")
    res = subprocess.run(
        "mv " + LFmapPath + "*healpy.fits " + LFmapPath + "/healpyFits",
        shell=True,
        capture_output=True,
    )
    print(res.stdout.decode())
    ...
    res = subprocess.run(
        "mv " + LFmapPath + "LFmap*.fits " + LFmapPath + "/fits",
        shell=True,
        capture_output=True,
    )
...
if __name__ == "__main__":
    ...
    optional.add_argument(
        "-p", "--path", nargs="?", default="./", help="Path to LFmap."
    )
    optional.add_argument(
        "-sf", "--startfreq", nargs="?", default="30.0", help="Start frequency."
    )
    optional.add_argument(
        "-ef", "--endfreq", nargs="?", default="80.0", help="End frequency."
    )
    optional.add_argument(
        "-d", "--deltafreq", nargs="?", default=0.1, help="Frequency step"
    )
