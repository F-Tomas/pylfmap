...
# this needs absolute path to the folder, getting it by finding the path of this module is the only method that I know of
pylfmap_path = os.path.dirname(importlib.util.find_spec("pylfmap").origin)
...
if exists(os.path.join(lfdir, hpfits_folder)):
    ...
    print('[INFO] Folder contaning converted fits does not exists. First run? Checking whether LFmap was compiled...')
    do_convert = True
    if exists(os.path.join(lfdir, 'LFmap')):
        ...
        if exists(os.path.join(lfdir, 'LFmap.c')):
            ...
            print('[INFO] LFmap folder does not exists. I will download the tar, untar it, compile and convert.')
...
if do_compile == True:
    ...
    result=subprocess.run(cd_to_lfmap_folder+'make', shell=True, capture_output=True)
    print(result.stdout.decode())
    print(result.stderr.decode())

if do_convert == True:
    print('[INFO] Generating and converting maps from 30 to 80 MHz in 0.1 MHz steps.')
    LFmap_healpyFitsConvertorAndGenerator.main(path=lfmap_abs_path, startFrequency = '30.0', endFrequency = '80.0', fdelta = 0.1, )
    print('[INFO] pylfmap was initialized with frequencies 30-80 MHz with step of 0.1 MHz. You can generate later your custom ranges and values.')
...
class LFmap:
    def __init__(self):
        ...
        print('[INFO] To generate different ones use LFmap.generate_new_frequencies(your new frequency).')
    
    def list_available_frequencies(self):
        ''' Lists all already converted maps.'''
        flist = [f.strip('LFmap_').strip('_healpy.fits') for f in os.listdir(lfdir+'/healpyFits/')]
        flist.sort()
        print(flist)
    ...
    def generate_new_frequencies(self, new_freq):
        ...
        if isinstance(new_freq, list) or isinstance(new_freq, np.ndarray):
            for f in new_freq:
                ...
                LFmap_healpyFitsConvertorAndGenerator.main(path=lfmap_abs_path, startFrequency=f, endFrequency=f, fdelta = 0.1)
        else:
            new_freq = '{:.1f}'.format(new_freq)
            print('[INFO] Generating new map at {} MHz'.format(new_freq))
            LFmap_healpyFitsConvertorAndGenerator.main(path=lfmap_abs_path, startFrequency=new_freq, endFrequency=new_freq, fdelta = 0.1)
        # if single value
        print('updated...')
    ...
    def generate(self, frequency):
        ...
        if exists(path_to_fit_file):
            return hp.read_map(
                path_to_fit_file, verbose=True
            )
        else:
            print("[ERROR] Required frequency '{}' does not exists! Use function LFmap.generate_new_frequencies() to generate it!".format(frequency))
